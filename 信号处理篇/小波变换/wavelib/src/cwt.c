...
static void wave_function(int nk, double dt, int mother, double param,
 ...
 if (mother == 0) {
    // MORLET小波
    // 如果传入的参数param小于0，则将其设置为默认值6.0
    if (param < 0.0) {
      param = 6.0;
    }
    // 计算归一化系数，这是为了确保小波函数的能量在所有尺度上保持一致
    norm = sqrt(2.0 * pi * scale1 / dt) * pow(pi, -0.25);

    // 计算小波函数的实部，对于nk/2 + 1个频率点
    for (k = 1; k <= nk / 2 + 1; ++k) {
      // 计算频率点与param的偏差，用于后续的高斯窗函数计算
      temp = (scale1 * kwave[k - 1] - param);
      // 计算高斯窗函数的指数部分
      expnt = -0.5 * temp * temp;
      // 计算实部，虚部设为0（Morlet小波是实函数）
      daughter[k - 1].re = norm * exp(expnt);
      daughter[k - 1].im = 0.0;
    }
    // 对于高于Nyquist频率的部分，将小波函数值设为0
    for (k = nk / 2 + 2; k <= nk; ++k) {
      daughter[k - 1].re = daughter[k - 1].im = 0.0;
    }
    // 计算Fourier因子，用于从小波尺度转换到对应的Fourier周期
    fourier_factor = (4.0 * pi) / (param + sqrt(2.0 + param * param));
    // 计算对应的周期
    *period1 = scale1 * fourier_factor;
    // 计算锥形影响区（cone of influence），用于确定在边界处的信号分析的可靠性
...
void cwavelet(const double *y, int N, double dt, int mother, double param,
  ...
  /*
    遍历所有尺度：对每一个尺度，使用小波母函数和对应的波数计算小波变换。

    小波函数生成：根据当前尺度和波数，计算小波“女儿”波形。
