...
int main(int argc, char** argv) {
    ...
    // DBSCANNanoCluster<double> dbscanNano;
    // PointCloud<double> nanoflann_pc;  //存储点云
    // nanoflann_pc.pts.clear();
    // nanoflann_pc.pts.resize(cloud_filtered->points.size());

    // int pidx=0;
    // for (auto p : cloud_filtered->points) {  //将降采样后的点云存入nanoflann_pc
    //     nanoflann_pc.pts.at(pidx).x = p.x;
    //     nanoflann_pc.pts.at(pidx).y = p.y;
    //     nanoflann_pc.pts.at(pidx).z = p.z;
    //     pidx++;
    // }

    // start_ms = clock();
    // std::vector<std::vector<unsigned long>> res_ = dbscanNano.extract(nanoflann_pc, 0.5, 100);
    // end_ms = clock();
