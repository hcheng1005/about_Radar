...
switch(scenario_type)
    case 'linear'
        ...
        initial_state(1).x = [100;0; 0; -10];        tbirth(1) = 1;   tdeath(1) = K;
    ...
    case 'nonlinear'
        ...
        initial_state(4).x = [-40; -10; 8; 0; pi/270];   tbirth(4) = 1;   tdeath(4) = K;
end
...
M = 10;                 %maximum number of hypotheses kept in MHT
...
for i = 1:nbirths
    h1 = plot(cell2mat(cellfun(@(x) x(1,i), objectdata.X, 'UniformOutput', false)), ...
        cell2mat(cellfun(@(x) x(2,i), objectdata.X, 'UniformOutput', false)), 'g-s', 'Linewidth', 4);
